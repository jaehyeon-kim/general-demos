...
class Product:
    ...
    def update(db: DbHelper):
        ...
        records = set(random.choices(products, k=int(len(products) * 0.2)))
        for r in records:
            r.price = r.price + 10
...
class User:
    ...
    def update(db: DbHelper):
        ...
        users = User.load(db)
        records = set(random.choices(users, k=int(len(users) * 0.2)))
        for r in records:
...
if __name__ == "__main__":
    ...
    db = DbHelper()
    ## update 20% of product and user records
    print(Product.update(db))
    print(User.update(db))
